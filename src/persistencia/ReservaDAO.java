...
public class ReservaDAO {
	...
	private ReservaDAO() throws ClassNotFoundException, SQLException, SelectException {
		...
		update=conexao.prepareStatement("update reserva set prazo_validade=?,cod_p=?,cod_voo=? where cod_res=?");
	...
	public void insert(Reserva reserva)throws InsertException{
		try {
			...
			insert.setInt(1, reserva.getCod_res());
			insert.setDate(2, reserva.getDta_validade());
	...
	public List<Reserva> selectAll() throws SelectException {
		...
		try {
			...
			while(rs.next()) {
				...
				reserva.setDta_validade(rs.getDate("prazo_validade"));
	...
	public void update(Reserva reserva)throws UpdateException{
		try {
			update.setDate(1, reserva.getDta_validade());
			update.setInt(2, reserva.getCod_p());
			update.setInt(3, reserva.getCod_voo());
